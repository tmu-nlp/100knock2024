...
const size_t kCharFeatureSize = 16;
...
void get_char_feature(int charset, const char *str, char *feature) {
  ...
  if (iscap) {
    snprintf(feature, kCharFeatureSize - 1, "C");
  } else if (clen == 1) {
    snprintf(feature, kCharFeatureSize - 1, "S%d", type1);
  } else if (type2 == -1) {
    snprintf(feature, kCharFeatureSize - 1, "M%d",  type1);
  } else {
    snprintf(feature, kCharFeatureSize - 1, "T%d-%d", type2, type1);
  }
}
} // end of anonmyous
...
bool NE::parse(Tree *tree) const {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    char *char_feature = tree->alloc(kCharFeatureSize);
