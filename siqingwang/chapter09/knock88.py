...
dh = 100  # Dimensionality of hidden state (increased)
...
class ImprovedCNNModel(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, output_size, embedding_matrix):
        ...
        self.conv1 = nn.Conv1d(embed_size, hidden_size, kernel_size=3, padding=1)  # Convolutional layer 1
        self.conv2 = nn.Conv1d(hidden_size, hidden_size, kernel_size=3, padding=1)  # Convolutional layer 2
        self.relu = nn.ReLU()
        self.max_pool = nn.AdaptiveMaxPool1d(1)  # Max pooling to size (N, hidden_size, 1)
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)  # Convolution: (N, dw, T) -> (N, dh, T)
        x = self.relu(x)
        x = self.conv2(x)  # Convolution: (N, dh, T) -> (N, dh, T)
        x = self.relu(x)
...
model = ImprovedCNNModel(vocab_size, dw, dh, L, embedding_matrix).cuda()
...
for epoch in range(epochs):
    ...
    print(f"Epoch {epoch + 1}/{epochs}, Training Loss: {total_loss / len(train_loader)}, Training Accuracy: {train_acc}")
