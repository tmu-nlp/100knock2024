    ...
    for chunks in sentences:
        for chunk in chunks:
            if chunk.has_verb():
                ...
                if base_verb and particles_and_phrases:
                    ...
                    patterns.append((base_verb, particles, phrases))
    return patterns
...
# 結果をファイルに保存
with open('predicate_case_patterns_with_phrases.txt', 'w', encoding='utf-8') as file:
    for verb, particles, phrases in patterns:
        file.write(f"{verb}\t{particles}\t{phrases}\n")

# コーパス中で頻出する述語と格パターンの組み合わせ
# sort predicate_case_patterns_with_phrases.txt | uniq -c | sort -nr | head

# 特定の動詞の格パターン
# grep '^行う' predicate_case_patterns_with_phrases.txt | sort | uniq -c | sort -nr
# grep '^なる' predicate_case_patterns_with_phrases.txt | sort | uniq -c | sort -nr
# grep '^与える' predicate_case_patterns_with_phrases.txt | sort | uniq -c | sort -nr